...
def _auto_reasoning_from_responsibility(resp: Any, *, party: str = "", category: str = "") -> str:
    ...
    if "indemnif" in low or "hold harmless" in low:
        return f"Shifts third‑party claim costs and related financial exposure to {p} through an indemnity obligation."
        ...
        *,
        ...
        try:
            ...
            if self.party_metadata:
                metadata_context = (
                    "\n\n--- PARTY METADATA (use to map contract labels to actual legal names — "
...
{page_text}"""
            ...
            message = f"📄 ANALYZING PAGE {page_num} with {_configured_llm_label()}..."
            self.logger.info(message)
            force_logger.info(message)  # Force terminal output
            ...
            if not groups or n_inner == 0:
                self.logger.warning(
                    "Page %s: 0 category blocks or 0 inner obligations — response (first 600 chars): %r",
                    page_num,
                    result_text[:600],
                )
    ...
    def _map_citations_by_similarity(self, consolidated_responsibilities: List[str], 
        ...
        for consolidated_resp in consolidated_responsibilities:
            ...
            if len(contributing_sources) == 1 and contributing_sources[0][1] > 0.85:
                # NON-MERGED: Single high-similarity match (>85%) → Use direct pagewise citation
                ...
                self.logger.debug(f"Direct citation: \"{consolidated_resp[:40]}...\" → Page {cite.get('page')} (sim:{contributing_sources[0][1]:.2f})")
                
            elif len(contributing_sources) > 1:
                # MERGED: Multiple sources → Keep all citations (already collected above)
                pages = [str(c["page"]) for c in citation_group]
                self.logger.info(f"Multi-source citation: \"{consolidated_resp[:50]}...\" → Pages {', '.join(pages)}")
                
            elif not citation_group and best_match[2] > 0.4:
                # FALLBACK: Best match with reasonable similarity → Use single citation
                cite = original_citations[best_match[0]]
                citation_group = [{
                    "page": cite.get("page", 1),
                    "section": cite.get("section", "Document")
                }]
                self.logger.debug(f"Fallback citation: \"{consolidated_resp[:40]}...\" → Page {cite.get('page')} (sim:{best_match[2]:.2f})")
                ...
                self.logger.warning(f"Positional citation: \"{consolidated_resp[:40]}...\" → Page {citation_group[0]['page']}")
...
CRITICAL RULE — MERGE ONLY NEAR-IDENTICAL SENTENCES:
...
ALLOWED to merge (near-identical phrasing of the same duty):
  ✅ "pay base rent monthly" + "pay base rent in equal monthly installments" → one item
  ✅ "maintain fire insurance" + "obtain and maintain fire insurance" → one item

FORBIDDEN to merge (different duties, even in the same category):
  ❌ "pay base rent" + "pay real estate taxes" — different financial obligations
  ❌ "pay base rent" + "pay operating expenses" — different line items
  ❌ "pay base rent" + "pay for alterations" — completely different topics
  ❌ "maintain insurance" + "pay base rent" — unrelated
  ❌ Any two obligations about different topics, even if the same party

FORBIDDEN to merge (same topic but different actions):
  ❌ "Pay Base Rent in advance monthly" + "Send all Rent payments to Landlord's Address"
     → These are different actions (payment schedule vs. payment method)
  ❌ "Pay Base Rent monthly" + "Pay prorated Rent for fractional months"
     → These cover different payment scenarios
  ❌ "Pay Base Rent monthly" + "Pay Real Estate Taxes as they become due"
     → These are completely different financial obligations
  ❌ "Pay first monthly installment on Effective Date" + "Pay subsequent installments on first day"
     → These are different timing conditions for the same payment type, keep separate

ALLOWED to merge (truly identical duty, minor wording only):
  ✅ "Keep HVAC systems in good condition and repair" + "Keep and maintain HVAC systems"
     → Same duty, different phrasing → merge to longer/more complete version
  ✅ "Pay Base Rent monthly without demand" + "Pay Base Rent in advance without demand monthly"
     → Same duty, minor wording difference → merge
        ...
        for it in items:
            ...
            if current and est_trial > max_input_tokens:
                ...
                if est_one > max_input_tokens:
                    self.logger.warning(
                        "Single responsibility text exceeds merge-plan token budget (~%.0f > %.0f) for %s / %s — "
    ...
    def _consolidate_party_items_map_reduce(
        ...
        *,
        ...
        while len(current) > 1 and round_idx < max_map_rounds:
            ...
            self.logger.info(
                "Merge-plan prompt ~%.0f tokens > budget %.0f for %s / %s (%d items) — token batching.",
                est_all,
                max_input_tokens,
                party,
                category_name,
...
• Do NOT include citation_groups in your response
• Citations will be mapped automatically using code logic
• Focus only on proper content consolidation
• Return valid JSON object only"""
                ...
                if response_text and response_text.strip():
                    ...
                    if json_match:
                        ...
                        if batch_resps:  # Process if we found valid data
                            ...
                            self.logger.info(f"Batch {batch_num} processed: {len(batch['responsibilities'])} → {len(batch_resps)} ({batch_reduction} merged)")
        ...
        self.logger.info(
            ...
            f"{total_items} → {final_count} responsibilities ({reduction_percent:.1f}% reduction)"
        )
    ...
    def _batch_consolidation_fallback(self, party: str, responsibilities: List[str],
        ...
        for group in consolidated_groups:
            if len(group["responsibilities"]) == 1:
                ...
                if len(pages) > 1:
                    self.logger.info(
                        f"Multi-source citation created: {len(group['responsibilities'])} responsibilities → Pages {', '.join(str(x) for x in pages)}"
                    )
        ...
        self.logger.info(
            ...
            f"{len(responsibilities)} → {len(final_responsibilities)} responsibilities "
...
• Merge obligations that are 90%+ similar in meaning and content
• Keep distinct obligations separate - different amounts, timing, or conditions stay separate
• For example: "Pay $500 estimated" vs "Pay actual amount if exceeds" are DIFFERENT obligations
• Only merge near-duplicates like "Pay rent monthly" + "Pay monthly rent" 
• Maintain reasonable consolidation ratio - don't over-reduce the total count
• Preserve specific details like amounts, percentages, timeframes, and conditions
• Clean citations (remove document names, keep only page + section)

OUTPUT INTEGRITY RULES (CRITICAL):
• Do NOT drop obligations. Every distinct input obligation must appear in output, either unchanged or merged.
• Do NOT write meta-reasoning about consolidation (avoid “merged/kept separate”, “unique”, “distinct scenario”, “granular language”, “clear expectations”).
• Reasoning must describe the responsibility itself (e.g., rent payment, tax allocation, insurance coverage, indemnity scope) and why it exists in the contract.
• Reasoning does NOT need to be 1:1 with responsibilities. Provide a small set of bullets (1–8) that collectively support all listed responsibilities.
• Ensure no responsibility is left unsupported; if needed, add another reasoning bullet rather than forcing 1:1 alignment.
...
• Eliminates repetition of party names
• Cleaner, more efficient format
• Easier to parse and process

IMPORTANT - CITATION HANDLING:
• Do NOT include citation_groups in your response
• Citations will be handled automatically by code logic
• Focus only on consolidating the content properly
• Return valid JSON object only"""
        ...
        if estimated_tokens > token_limit:
            message = f"🚨 TOKEN LIMIT EXCEEDED: Estimated {estimated_tokens:.0f} tokens > {token_limit} limit - Switching to BATCH LLM processing for {party} in '{category_name}'"
        ...
        try:
                        ...
                        if "Owner Responsibility" in parsed_data:
                            ...
                            if content_loss_count > 0:
                                ...
                                if content_loss_percent > 15:
                                    message = f"🚨 CONTENT LOSS THRESHOLD EXCEEDED ({content_loss_percent:.1f}% > 15%) - Trying BATCH LLM processing for {party} in '{category_name}'"
                                    self.logger.warning(message)
                                    print(message, flush=True)  # Force terminal output with flush
                                    return self._batch_llm_consolidation(
                                ...
                                message = f"✅ GOOD CONSOLIDATION: {party} in '{category_name}' - {input_count} → {output_count} ({reduction_percent:.1f}% reduction, no content loss)"
                                self.logger.info(message)
                                print(message, flush=True)  # Force terminal output with flush
            ...
            if len(responsibilities) < len(citations):
                ...
                self.logger.info(
                    f"Fallback: {len(citations)} citations → {len(responsibilities)} "
            ...
            if len(responsibilities) < len(citations):
                # Responsibilities were merged - distribute citations appropriately
                self.logger.info(
                    f"Exception fallback: {len(citations)} citations → {len(responsibilities)} "
...
CONSOLIDATION RULES:
1. **Very similar responsibilities** → Drop one duplicate
   - Example: "Pay rent monthly" + "Pay monthly rent" → Keep one

2. **Little bit different** → Merge but preserve BOTH meanings
   - Example: "Pay rent monthly" + "Pay rent in advance" → "Pay rent monthly in advance"
   - CRITICAL: Both concepts (monthly + in advance) must be preserved

3. **Different meanings** → Keep separate
...
CONSOLIDATION RULES:
1. **Exact duplicates only** → Drop one duplicate
2. **Very similar with same purpose** → Merge but keep CONCISE and READABLE
3. **Different legal purposes or rights** → ALWAYS keep separate
...
EXAMPLES:
✅ MERGE: "Pay Base Rent monthly" + "Pay Base Rent in advance" → "Pay Base Rent monthly in advance"
✅ DROP: "Pay Base Rent" + "Pay monthly Base Rent" → Keep one (identical)  
❌ NEVER MERGE: "Pay expenses" vs "Audit expense records" (different purposes: payment vs rights)
❌ NEVER MERGE: "Pay rent" vs "Rent abatement" (different: obligation vs protection)
❌ NEVER MERGE: "Insurance required" vs "Indemnification" (different legal concepts)
...
Return ONLY the JSON array."""
    ...
    def _accuracy_focused_consolidation(self, results: List[Dict[str, Any]], document_name: str) -> List[Dict[str, Any]]:
        ...
        1. Very similar responsibilities → Drop duplicate
        2. Little bit different → Merge but preserve both meanings  
    ...
    def _apply_user_consolidation_rules(self, resp_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        1. Very similar → Drop duplicate
        2. Little bit different → Merge but preserve meanings
    ...
    def _find_and_merge_similar(self, base_item: Dict[str, Any], resp_list: List[Dict[str, Any]], 
        ...
        for j, other_item in enumerate(resp_list):
            ...
            if similarity > 0.97:  # True duplicate (only punctuation/whitespace difference) → Drop
                processed_indices.add(j)
                continue

            elif similarity > 0.92:  # Near-identical phrasing of the same duty → Merge
                merged_responsibility = self._merge_preserving_meanings(
                    merged_responsibility, other_item["responsibility"]
                )
                processed_indices.add(j)
            # else: similarity <= 0.92 → keep as separate obligation (DO NOT MERGE)
    ...
    def _calculate_semantic_similarity(self, text1: str, text2: str) -> float:
        """
        Jaccard word overlap only (no subject-based boosting — that caused false merges).
    ...
    def process_document(self, pdf_path: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if category_results:
                    self.logger.info("🔄 Processing obligations with auto-generated keywords...")
                    ...
                    self.logger.info(f"📋 Flattened to {len(individual_obligations)} individual searchable obligations")
                    ...
                    self.logger.info(f"🎯 Individual obligation index: {num_individual_indexed} obligations with auto-generated keywords indexed")
                    ...
                    try:
                        ...
                        self.logger.info(f"📦 Legacy index: {num_legacy_indexed} chunks (old approach) at {chroma_path_legacy}")
                        ...
                        self.logger.info(f"📂 Legacy category index: {num_cat_indexed} categories at {chroma_path_legacy}")
            ...
            if use_sections:
                ...
                message = f"🎉 PROCESSING COMPLETE! Pages: {len(page_texts)}, obligations: {_count_inner_obligation_rows(all_category_groups)}, consolidated: {consolidated_ob_count}"
                self.logger.info(message)
                force_logger.info(message)  # Force terminal output
            return result_path
        except Exception as e:
